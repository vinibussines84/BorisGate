...
class SharkBankWebhookController extends Controller
{
    public function __invoke(Request $request, WalletService $wallet)
    {
        try {
            ...
            Log::info("📩 Webhook SharkBank PIX recebido", ['payload' => $raw]);
            ...
            if (!$tx) {
                Log::warning("⚠️ TX não encontrada para webhook SharkBank", [
            ...
            Log::error("🚨 ERRO NO WEBHOOK SHARKBANK PIX", [
                'error'   => $e->getMessage(),
