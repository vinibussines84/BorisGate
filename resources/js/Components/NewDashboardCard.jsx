...
/* --- Linha animada no topo (mesma do PaymentAccountCard, em emerald) --- */
...
/* =============================
   Card Principal (cores iguais)
============================= */
export default function NewDashboardCard({ minHeight = 80, initialBalances = {} }) {
  const [showBalance, setShowBalance] = useState(true);
  const [loading, setLoading] = useState(true);
  const [isRefreshing, setIsRefreshing] = useState(false);
  const [saldoRetido, setSaldoRetido] = useState(Number(initialBalances.amount_retained ?? 0));
  const [bloqueioCautelar, setBloqueioCautelar] = useState(Number(initialBalances.blocked_amount ?? 0));
  ...
  const loadBalances = async ({ initial = false } = {}) => {
    try {
      ...
      if (!res.ok || !json?.success) throw new Error(json?.message || "Erro ao obter dados.");

      const data = json.data || {};
      setSaldoRetido(Number(data.amount_retained ?? 0));
      setBloqueioCautelar(Number(data.blocked_amount ?? 0));
    } catch (err) {
      setError(err?.message || "Falha ao carregar saldos.");
    } finally {
      setLoading(false);
  ...
  return (
    ...
    >
        ...
        <div className="inline-flex items-center gap-2 rounded-full border border-neutral-700 bg-neutral-900/60 px-3 py-1.5">
          <span className="inline-flex items-center gap-1.5">
            ...
            <span className="text-[11px] font-medium text-neutral-300 tracking-wide">
              {error ? "Erro ao atualizar" : "Restrições de saldo"}
            </span>
          </span>
        </div>
      ...
      {/* Corpo (cards internos nas mesmas cores) */}
        ...
        <div className="group rounded-xl border border-neutral-800 bg-neutral-900/40 hover:bg-neutral-800/40 transition-all duration-200 px-3 py-3">
          <div className="flex items-start justify-between">
            ...
            {loading ? (
              ...
              <Valor v={saldoRetido} dim={isRefreshing} />
            )}
          </div>
          <p className="mt-1 text-[11px] text-neutral-400 flex items-center gap-1 leading-tight">
            <Lock size={12} className="text-neutral-300" /> Temporariamente indisponível
          </p>
        </div>
        ...
        <div className="group rounded-xl border border-neutral-800 bg-neutral-900/40 hover:bg-neutral-800/40 transition-all duration-200 px-3 py-3">
          <div className="flex items-start justify-between">
            ...
            {loading ? (
              ...
              <Valor v={bloqueioCautelar} dim={isRefreshing} />
            )}
          </div>
          <p className="mt-1 text-[11px] text-neutral-400 flex items-center gap-1 leading-tight">
            <ShieldAlert size={12} className="text-neutral-300" /> Análise de Segurança /{" "}
            <span className="text-red-400 font-semibold">Risco</span>
          </p>
        </div>
      </div>
    </section>
