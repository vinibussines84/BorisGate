/** ✅ COMPLETO + VISUAL UNIFICADO (PRETO + VERDE #02fb5c) + LOGS AUTOMÁTICOS **/
...
export default function WebhooksIndex({ webhooks = [] }) {
  ...
  const handleSubmit = (formData) => {
    router.post(route("webhooks.store"), formData, {
      preserveScroll: true,
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      setToast({ message: "✔️ Webhook reenviado com sucesso!", type: "success" });
      fetchLogs();
    } catch {
  ...
  const fetchLogs = async () => {
    setLoadingLogs(true);
    try {
      const res = await fetch("/webhooks/logs", { headers: { Accept: "application/json" } });
        ...
        <div className="max-w-6xl mx-auto space-y-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
            <div className="flex-1 bg-[#0b0b0b]/90 rounded-full px-8 py-5 flex items-center justify-between shadow-[0_0_20px_-8px_rgba(0,0,0,0.8)] border border-white/10">
              ...
              <div className="hidden sm:flex bg-[#111]/70 rounded-full p-1 border border-white/10 ml-6">
                {["webhooks", "logs"].map((tab) => (
                  ...
                  >
                    {tab === "webhooks" ? "Webhooks" : "Eventos enviados"}
                  </button>
                ))}
              </div>
            </div>
            ...
            <button
              ...
              className="flex items-center gap-3 bg-[#02fb5c] text-black font-semibold px-7 py-4 rounded-full shadow-[0_0_15px_rgba(2,251,92,0.4)] hover:brightness-110 active:scale-95 transition"
            >
              <PlusCircle size={20} />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-[#0b0b0b]/90 rounded-2xl border border-white/10 p-5 flex items-center gap-4 shadow-[0_0_15px_-5px_rgba(0,0,0,0.8)]">
              ...
              <div>
                ...
                <h3 className="text-xl font-bold text-white">{webhooks.length}</h3>
              </div>
            </div>
            ...
            <div className="bg-[#0b0b0b]/90 rounded-2xl border border-white/10 p-5 flex items-center gap-4 shadow-[0_0_15px_-5px_rgba(0,0,0,0.8)]">
              ...
              <div>
                ...
                <h3 className="text-xl font-bold text-white">{logs.length ?? 0}</h3>
              </div>
            </div>
          </div>
                      ...
                      webhooksPaginated.items.map((hook) => (
                        ...
                        >
                          ...
                          <td className="py-3 pr-4 text-gray-300 capitalize whitespace-nowrap">
                            <div className="flex items-center gap-2">
                              <ArrowLeftRight size={16} className="text-[#02fb5c]" />
                              {hook.type}
                            </div>
                          </td>
                      ...
                      {logsPaginated.items.map((log) => (
                        ...
                        >
                          ...
                          <td className="py-3 text-right">
                            <button
                              onClick={() => handleAskResend(log.id)}
